...
```bash
StandaloneFunction$ sam local invoke helloFromLambdaFunction --no-event
```
...
Resources:
  ...
  helloFromLambdaFunction:
    ...
    Properties:
      CodeUri: ./src/StandaloneFunction/
      Handler: StandaloneFunction::StandaloneFunction.Function::FunctionHandler
      Runtime: dotnet6
      MemorySize: 256
      Timeout: 100
      DeadLetterQueue:
...
```bash
StandaloneFunction$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
StandaloneFunction$ dotnet test test/StandaloneFunction.Tests/StandaloneFunction.Tests.csproj 
```
...
```bash
aws cloudformation delete-stack --stack-name StandaloneFunction
```
