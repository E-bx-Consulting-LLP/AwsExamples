...
```bash
ServerlessApi$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
ServerlessApi$ sam local start-api
ServerlessApi$ curl http://localhost:3000/
```
...
```bash
ServerlessApi$ sam logs -n NetCodeWebAPIServerless --stack-name ServerlessApi --tail
```
...
```bash
ServerlessApi$ dotnet test test/ServerlessApi.Tests/ServerlessApi.Tests.csproj 
```
...
```bash
aws cloudformation delete-stack --stack-name ServerlessApi
```
